 ...
 * @param array     $args
 *
 * @return SOLID_CT_Table
 */
function solid_ct_register_table($name, $args)
{
    ...
    $ct_table = new SOLID_CT_Table($name, $args);
 ...
 * @since 1.0.0
 *
 * @param SOLID_CT_Table|string $object CT_Table object or CT_Table name
 *
 * @return SOLID_CT_Table $ct_table
 */
function solid_ct_setup_table($object)
{
 ...
 * @return SOLID_CT_Table $ct_table
 */
function solid_ct_reset_setup_table()
{
 ...
 *
 * @param SOLID_CT_Table|string $name CT_Table object or CT_Table name
 *
 * @since 1.0.0
 *
 * @return SOLID_CT_Table
 */
function solid_ct_get_table_object($name)
{
 ...
 * @since 1.0.0
 *
 * @param SOLID_CT_Table $ct_table
 *
 * @return array
 */
function solid_ct_get_table_labels($ct_table)
{
 ...
 * @return array
 */
function solid_ct_get_table_fields($ct_table)
{
 ...
 * @since 1.0.0
 */
function solid_ct_populate_roles()
{
    ...
    $capabilities = solid_ct_get_table_capabilities($args);
 ...
 */
function solid_ct_get_table_capabilities($args)
{
    ...
    if ($args->map_meta_cap)
        solid_ct_meta_capabilities($capabilities);
 ...
 */
function solid_ct_meta_capabilities($capabilities = null)
{
    global $ct_meta_caps;
 ...
 */
function solid_ct_get_object($object = null, $output = OBJECT, $filter = 'raw')
{
    ...
    if (is_object($object)) {
        ...
        $object = solid_ct_get_object_instance($object->$primary_key);
    } else {
        $object = solid_ct_get_object_instance($object);
    }
 ...
 */
function solid_ct_get_object_instance($object_id = null)
{
 ...
 */
function solid_ct_clean_object_cache($object)
{
    ...
    $object = solid_ct_get_object($object);
    if (empty($object))
        return;
 ...
 */
function solid_ct_update_object($object_data = array(), $wp_error = false)
{
    ...
    $object = solid_ct_get_object($object_data[$primary_key], ARRAY_A);
    ...
    return solid_ct_insert_object($object_data, $wp_error);
}
 ...
 *
 * @global wpdb $wpdb WordPress database abstraction object.
 * @global SOLID_CT_Table $ct_table Custom Tables CT_Table object type.
 *
 ...
 */
function solid_ct_insert_object($object_data, $wp_error = false)
{
    ...
    if (!empty($object_data[$primary_key])) {
        ...
        $object_before = solid_ct_get_object($object_id);
        $original_object_data = solid_ct_get_object($object_id, ARRAY_A);
    ...
    if (!empty($object_data['meta_input']) && $ct_table->meta) {
        foreach ($object_data['meta_input'] as $field => $value) {
            solid_ct_update_object_meta($object_id, $field, $value);
        }
    }

    solid_ct_clean_object_cache($object_id);

    $object = solid_ct_get_object($object_id);
    ...
    if ($update) {
        ...
        $object_after = solid_ct_get_object($object_id);
     ...
     */
    do_action('solid_ct_save_object', $object_id, $object, $update);
 ...
 */
function solid_ct_delete_object($object_id = 0, $force_delete = false)
{
    ...
    $ct_object = solid_ct_get_object($ct_object);
    ...
    if ($ct_table->meta) {
        solid_ct_delete_object_meta($object_id, '_wp_trash_meta_status');
        solid_ct_delete_object_meta($object_id, '_wp_trash_meta_time');
        ...
        foreach ($object_meta_ids as $mid) {
            solid_ct_delete_metadata_by_mid('post', $mid);
        }
    }
    ...
    solid_ct_clean_object_cache($ct_object);
 ...
 */
function solid_ct_add_object_meta($object_id, $meta_key, $meta_value, $unique = false)
{
 ...
 */
function solid_ct_delete_object_meta($object_id, $meta_key, $meta_value = '')
{
 ...
 */
function solid_ct_get_object_meta($object_id, $meta_key = '', $single = false)
{
    ...
    if (!$meta_cache) {
        $meta_cache = solid_ct_update_meta_cache($ct_table->meta->name, array($object_id));
 ...
 *                  false on failure.
 */
function solid_ct_update_object_meta($object_id, $meta_key, $meta_value, $prev_value = '')
{
    ...
    if (empty($prev_value)) {
        $old_value = solid_ct_get_object_meta($object_id, $meta_key);
    ...
    if (empty($meta_ids)) {
        return solid_ct_add_object_meta($object_id, $raw_meta_key, $passed_value);
    }
 ...
 */
function solid_ct_update_meta_cache($meta_type, $object_ids)
{
 ...
 */
function solid_ct_get_metadata_by_mid($meta_type, $meta_id)
{
 ...
 */
function solid_ct_delete_metadata_by_mid($meta_type, $meta_id)
{
    ...
    if ($meta = solid_ct_get_metadata_by_mid($meta_type, $meta_id)) {
 ...
 * @return mixed
 */
function solid_ct_has_meta($object_id)
{
 ...
 */
function solid_ct_meta_form($object = null)
{
 ...
 * @return string $url
 */
function solid_ct_get_list_link($name)
{
    global $ct_registered_tables;
 ...
 * @return string
 */
function solid_ct_get_edit_link($name, $object_id = 0)
{
    global $ct_registered_tables;
 ...
 * @return string
 */
function solid_ct_get_delete_link($name, $object_id = 0)
{
    global $ct_registered_tables;
