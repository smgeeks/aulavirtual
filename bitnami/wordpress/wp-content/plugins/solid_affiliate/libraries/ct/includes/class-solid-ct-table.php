...
if (!class_exists('SOLID_CT_Table')) :

    class SOLID_CT_Table
    {
        ...
        public function set_props($args)
        {
            ...
            $this->cap = solid_ct_get_table_capabilities((object) $args);
            ...
            $labels = (array) solid_ct_get_table_labels($this);
            ...
            if (isset($args['db'])) {
                if (is_array($args['db'])) {
                    ...
                    $this->db = new SOLID_CT_DataBase($this->name, $args['db']);
                } else if ($args['db'] instanceof SOLID_CT_DataBase || is_subclass_of($args['db'], 'CT_DataBase')) {
                    // Table as custom object
                    $this->db = $args['db'];
                }
            } else {
                // Default database initialization
                $this->db = new SOLID_CT_DataBase($this->name, $args);
            }
            ...
            if (isset($args['views']) && is_array($args['views'])) {
                ...
                foreach ($args['views'] as $view => $view_args) {
                    ...
                    if (is_array($view_args)) {
                        ...
                        switch ($view) {
                            case 'list':
                                $views[$view] = new SOLID_CT_List_View($this->name, $view_args);
                                break;
                            case 'add':
                                $views[$view] = new SOLID_CT_Edit_View($this->name, $view_args);
                                break;
                            case 'edit':
                                $views[$view] = new SOLID_CT_Edit_View($this->name, $view_args);
                                break;
                            default:
                                $views[$view] = new SOLID_CT_View($this->name, $view_args);
                                break;
                        }
                    } else if ($view_args instanceof SOLID_CT_View || is_subclass_of($view_args, 'CT_View')) {
                ...
                $this->views = (object) array(
                    'list' => new SOLID_CT_List_View($this->name, $views_defaults['list']),
                    'add' => new SOLID_CT_Edit_View($this->name, $views_defaults['add']),
                    'edit' => new SOLID_CT_Edit_View($this->name, $views_defaults['edit']),
                );
            }

            // Meta data
            if (in_array('meta', $this->supports)) {
                $this->meta = new SOLID_CT_Table_Meta($this);
            }
        }
    }
