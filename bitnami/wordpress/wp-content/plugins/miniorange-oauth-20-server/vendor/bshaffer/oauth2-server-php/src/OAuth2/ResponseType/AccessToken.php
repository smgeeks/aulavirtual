...
class AccessToken implements AccessTokenInterface {
	...
	protected function generateAccessToken() {
		$len = (bool) get_option( 'mo_oauth_server_token_length' ) ? (int) get_option( 'mo_oauth_server_token_length' ) : (int) 32;
		if ( $len < 32 ) {
			$len = 32;
		}
		// $token_length = ($len % 2) === 1 ? ($len+1)/2 : $len/2;
		$token_length = ( $len >= 127 ) ? 127 : $len;
		if ( function_exists( 'openssl_random_pseudo_bytes' ) ) {
			$randomData = openssl_random_pseudo_bytes( $token_length / 2 );
			if ( $randomData !== false && strlen( $randomData ) === $token_length / 2 ) {
				return bin2hex( $randomData );
			}
		}
		if ( @file_exists( '/dev/urandom' ) ) { // Get 100 bytes of random data
			$randomData = file_get_contents( '/dev/urandom', false, null, 0, $token_length / 2 );
			if ( $randomData !== false && strlen( $randomData ) === $token_length / 2 ) {
				return bin2hex( $randomData );
			}
		}
		...
		return substr( hash( 'sha512', $randomData ), 0, $token_length );
	}
