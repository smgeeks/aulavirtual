...
class AuthorizeController implements AuthorizeControllerInterface {
	...
	public function handleAuthorizeRequest( RequestInterface $request, ResponseInterface $response, $is_authorized, $user_id = null ) {
		...
		$this->clientStorage->authorizeClient( $this->client_id, $user_id );
	...
	public function validateAuthorizeRequest( RequestInterface $request, ResponseInterface $response ) {
		...
		if ( $this->config['enforce_state'] && ( ! $state || empty( $state ) ) ) {
			// $response->setRedirect($this->config['redirect_status_code'], $redirect_uri, null, 'invalid_request', 'The state parameter is required');
			$response->setError( 400, 'invalid_state', 'The state parameter supplied is invalid or empty' );
		...
		$user_id = get_current_user_id();
		if ( $this->clientStorage->checkIfAlreadyAuthorized( $client_id, $user_id ) ) {
