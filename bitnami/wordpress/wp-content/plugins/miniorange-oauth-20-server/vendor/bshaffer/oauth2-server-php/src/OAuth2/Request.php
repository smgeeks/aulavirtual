...
class Request implements RequestInterface {
	...
	private function getHeadersFromServer( $server ) {
		...
		if ( isset( $server['PHP_AUTH_USER'] ) ) {
			...
			if ( isset( $server['HTTP_AUTHORIZATION'] ) ) {
				$authorization_header = $server['HTTP_AUTHORIZATION'];
			} elseif ( isset( $server['REDIRECT_HTTP_AUTHORIZATION'] ) ) {
				$authorization_header = $server['REDIRECT_HTTP_AUTHORIZATION'];
			...
			if ( null !== $authorization_header ) {
				...
				if ( 0 === stripos( $authorization_header, 'basic' ) ) {
					$exploded = explode( ':', base64_decode( substr( $authorization_header, 6 ) ) );
					if ( count( $exploded ) == 2 ) {
	...
	public static function createFromGlobals() {
		...
		$request = new $class( $_GET, $_POST, array(), $_COOKIE, $_FILES, $_SERVER ); //phpcs:ignore WordPress.Security.NonceVerification
		...
		if ( 0 === strpos( $contentType, 'application/x-www-form-urlencoded' )
			...
			&& in_array( strtoupper( $requestMethod ), array( 'POST', 'PUT', 'DELETE' ), true )
		) {
