...
class AuthorizationCode implements AuthorizationCodeInterface {
	...
	public function getAuthorizeResponse( $params, $user_id = null ) {
		if ( isset( $params['scope'] ) && strpos( $params['scope'], 'openid' ) !== false ) {
			$enable_oidc = (bool) get_option( 'mo_oauth_server_enable_oidc' ) ? get_option( 'mo_oauth_server_enable_oidc' ) : 'on';
			if ( $enable_oidc === 'off' ) {
				wp_die( 'OpenID Connect support has been disabled by the server administrator. <br><b>ERROR:</b> ERR_OIDC_DISABLED' );
				exit();
			}
		}
