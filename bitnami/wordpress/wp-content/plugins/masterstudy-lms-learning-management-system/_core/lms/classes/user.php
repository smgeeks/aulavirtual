...
class STM_LMS_User {
	...
	public static function has_course_access( $course_id, $item_id = '', $add = true ) {
		...
		$membership_expired  = ( STM_LMS_Subscriptions::subscription_enabled() && $membership_level && 'expired' == $membership_status && ! $not_in_membership && ! $is_bought && ! $is_free && ! $in_enterprise && $in_bundle && empty( $course[0]['for_points'] ) ); // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
		$membership_inactive = ( STM_LMS_Subscriptions::subscription_enabled() && $membership_level && 'active' !== $membership_status && 'expired' !== $membership_status && ! $not_in_membership && ! $is_bought && ! $is_free && ! $my_course && ! $in_enterprise && $in_bundle && empty( $course[0]['for_points'] ) ); // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
