...
<div id="stm-lms-register"
    ...
    <form @submit.prevent="register()" class="stm_lms_register_wrapper">
        ...
        <div class="row">
            <div v-for="(profileField, index) in profileDefaultFieldsForRegister"
                 :class="index === 'description' ? 'col-md-12' : 'col-md-6'">
                <div class="form-group">
                    <label class="heading_font" v-html="profileField.label"></label>
                    <input class="form-control" v-if="index !== 'description'" type="text" v-model="profileField.value"
                           :placeholder="profileField.placeholder" :required="profileField.required"/>
                    <textarea class="form-control" v-if="index === 'description'" v-model="profileField.value"
                              :placeholder="profileField.placeholder" :required="profileField.required"></textarea>
                </div>
            </div>
        </div>
        <div class="row additional-fields" v-if="additionalRegisterFields.length"
            ...
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <?php STM_LMS_Templates::show_lms_template('account/v1/form_builder/date'); ?>
    ...
    <transition name="slide-fade">
        <div class="stm-lms-message" v-bind:class="{status: status != 'success'}" v-if="message &&  status != 'success'">
            {{ message }}
        </div>
    </transition>

    <div class="stm-modal"  v-bind:class="{'success': status === 'success'}">
        <div class="stm-moldal-content">
            <div class="stm-modal-title">
                <?php esc_html_e('Confirma tu correo electrónico', 'masterstudy-lms-learning-management-system'); ?>
            </div>
            <div class="stm-modal-text">
                <?php esc_attr_e('Para terminar tu registro ves at u bandeja de entrada y verifica tu correo electrónico.'); ?>
            </div>
            <div class="stm-modal-img"></div>
            <div class="stm-modal-button">
                <a class="stm-popup_btn" href="/user-account"><?php esc_html_e('Volver al inicio', 'motors-child'); ?></a>
            </div>
        </div>
    </div>
